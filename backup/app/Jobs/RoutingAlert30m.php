...
class RoutingAlert extends Job
{
    ...
    public function handle(){
        $log_string ='';
        /*$route_sp_ids = SPRoutingAlert::where('routing_before', '30')->->where(function ($query) {
                $query->whereIn('status', ['SEARCHING','ACCEPTED']);
            })->get()->unique('subscription_id');*/
            
            
        $route_sp_ids = SPRoutingAlert::select('id','subscription_id','sub_category_id','routing_before','status')->where('status', 'SEARCHING')->orWhere('status', 'ACCEPTED')->where('routing_before', '30')->get()->unique('subscription_id');
